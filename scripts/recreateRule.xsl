...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="descendant::div[@type = 'rule']"/>
        <xsl:result-document href="./output/rule/Index.html">
            <xsl:for-each select="descendant::div[@type = 'rule']">
                <html>
                    <body>
                        <xsl:variable name="rule" select="@n"/>
                        <a href="Rule_{$rule}.html"> Rule <xsl:value-of select="$rule"/>
                        </a>
                        <br/>
                    </body>
                </html>
    ...
    <xsl:template match="div[@type = 'rule']">
        <xsl:message>
            <xsl:value-of select="@n"/>
        </xsl:message>
        <xsl:result-document href="./output/rule/Rule_{@n}.html">
            <html>
                <head>
                    <link rel="stylesheet" href="../style/style.css" type="text/css"/>
                    <link href="https://fonts.googleapis.com/css?family=Roboto+Slab|Roboto"
                        rel="stylesheet"/>
                </head>
                <body>
                    <xsl:apply-templates/>
    ...
    <xsl:template match="div[@type = 'concordance']">
        <table title="concordance">
            <head>
